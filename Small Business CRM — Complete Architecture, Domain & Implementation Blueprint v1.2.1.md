...
**Version:** 1.2.1
**Status:** Architecture baseline amended for Roadmap v1.3.1
...
## v1.2.1 Amendment Authority

This blueprint preserves architectural rationale. **Implementation sequencing is superseded by `CRM_IMPLEMENTATION_ROADMAP_v1.3.1.md` and the Phase 0–14 execution authorities.** Where older historical prose conflicts with the v1.3.1 roadmap, the v1.3.1 roadmap wins. Canonical definitions may be summarised here more than once, but only the current normative contract controls implementation.

The v1.2.1 amendment also freezes these corrections: Attention stores only `none` / `waiting_on_client` / `waiting_on_us`; follow-up is Task-derived; pause is orthogonal; Cloudflare Workers with Static Assets is the frontend deployment target; money, trusted mutations, append-only activity, quote snapshots, outbound-message uncertainty, KPI formulas, POPIA operations, complete recovery, timezone/contact semantics and toolchain reproducibility are mandatory Phase 0 contracts.

The v1.2.1 security/loop amendment additionally freezes: trusted SQL functions default to `SECURITY INVOKER`; `SECURITY DEFINER` requires documented elevation, safe explicit search path, fully qualified sensitive references, internal actor/role/status/domain/AAL checks and selective EXECUTE grants after revoking inappropriate PUBLIC/default access. Authorization uses protected `profiles.role` / `profiles.status`, never user-controlled `raw_user_meta_data`; public signup is disabled; privileged Owner/Admin actions require current-session AAL2; browser/Data-API reporting views use `security_invoker=true` or an equivalently secured trusted boundary. Hard bounce remediation returns current actionable work to `waiting_on_us` with one idempotent corrective Task while preserving Quote history. Recovery treats managed Auth identity reconstruction, password/re-invite/MFA expectations and Storage mapping explicitly rather than assuming an ordinary application-data dump is sufficient. The autonomous loop hashes all frozen normative authorities plus completed/current phase authorities and has a dedicated unexpected-drift stop.

The dependency/toolchain amendment is governed by `DEPENDENCY_BASELINE_v1.0.0.md`: Bun is the package manager/local runner, SvelteKit/Vite owns application builds, Cloudflare uses adapter-cloudflare + Wrangler + `wrangler.jsonc`, Tailwind 4 + shadcn-svelte + Lucide form the UI baseline, Supabase CLI is project-local, SendPulse uses a project-owned REST adapter, primary tests are Vitest + Playwright, and exact direct package versions are frozen only after the Phase 1 compatibility proof.
...
- one Cloudflare Workers application using Static Assets;
...
`follow_up_scheduled` is deliberately **not** an attention value: scheduled work belongs to Tasks and can coexist with `waiting_on_client` or `waiting_on_us`. An intentional pause is also orthogonal and is represented by `paused_at`, `pause_reason`, and optional `resume_at`.
...
Cloudflare Workers with Static Assets is the deployment target. Plan limits, pricing, included requests, build behavior and custom-domain constraints are operational inputs and must be re-verified against current Cloudflare documentation during provisioning. The architecture must not depend on a stale free-tier quota.

Because the CRM frontend can remain static-first, normal asset delivery should be inexpensive and cache-friendly. Application architecture must nevertheless remain correct if Cloudflare pricing or quotas change; commercial plan selection is a deployment decision, not domain law.
...
Pipeline position does not tell us who owes attention. Attention stores only that responsibility:
...
No party is currently being waited on. This does not imply there is no Task.
...
## Scheduled follow-up is derived

A future follow-up is represented by an open Task, not by `attention_state`. A Lead may therefore be `waiting_on_client` while also having a follow-up task due next Tuesday. Derived projections may expose `has_follow_up`, `next_task_due_at`, and `is_overdue`.

## Pause is orthogonal

An intentionally paused opportunity uses:

- `paused_at`;
- required `pause_reason`;
- optional `resume_at`.

Pausing must not overwrite the last meaningful attention owner. Resuming clears the pause fields through a trusted transition and records Activity.
...
Never use floating-point arithmetic for money. The Phase 0 Money Contract is normative: v1 uses two-decimal ISO-4217 currencies, deterministic decimal `ROUND_HALF_UP`, line-level rounding before document aggregation, and server/database-authoritative totals.

A finalised/sent Quote revision must also persist immutable seller, recipient and commercial snapshots plus `document_template_version`, `document_generator_version`, artifact hash/path, and acceptance evidence fields. Historical rendering must not depend on mutable Lead/Client/Contact records. `lead_id` remains anchored to the originating opportunity; `client_id` is nullable until a trusted conversion/link action populates it.
...
`submission_unknown` is a first-class uncertainty state: the provider may have accepted the request but the CRM did not receive definitive acknowledgement. It must not be auto-retried as though failure were certain. Provider identifiers/events may reconcile it; otherwise an authorised operator uses a controlled retry/reconciliation action.

Each logical message has a deterministic `idempotency_key`; each provider attempt is recorded separately so retries do not erase evidence.
...
Do not permit normal users to edit historical events. Database privileges/RLS/triggers/functions must enforce that ordinary application roles cannot UPDATE or DELETE Activity rows. A correction appends a new correction/reversal Activity referencing the earlier event. Exceptional legal/privacy retention actions are separate privileged audited procedures, not ordinary timeline editing.

Security/operational audit evidence should also cover privileged role changes, user suspension/reactivation, integration/configuration changes, exceptional reopen/correction, restore/migration operations and other high-risk administrative actions.
...
```text
Create logical outbound_message + deterministic idempotency key
        ↓
pending
        ↓
claim + record send attempt
        ↓
submitting
        ├── definitive provider acknowledgement → submitted
        ├── definitive rejection/error        → failed
        └── acknowledgement uncertain         → submission_unknown
                                                ↓
                                      provider reconciliation
                                                ↓
                                   submitted / controlled retry
        ↓
Provider webhook
        ↓
delivered / bounced / engagement events
```

The local database transaction cannot atomically commit the remote provider's acceptance. The design therefore models uncertainty rather than pretending every timeout is a failure. This keeps external API ambiguity from corrupting the Quote domain or causing uncontrolled duplicate sends.
...
5. claim message and create a send-attempt record;
6. invoke SendPulse;
7. if definitive acknowledgement arrives, mark message Submitted and persist provider identifier;
8. if acknowledgement is uncertain, mark `submission_unknown` and stop automatic progression/retry pending reconciliation;
9. only after the domain's definitive send-success boundary is satisfied, mark quote Sent;
10. create exactly one follow-up Task;
11. update lead to Decision;
12. set Waiting on Client;
13. create Activity events.

Definitive failure leaves enough state to retry safely. Ambiguous failure must preserve uncertainty and must not blindly resend.
...
# 99. Historical Implementation Phases — SUPERSEDED FOR EXECUTION

> **Execution note:** this section is retained as architectural history only. The authoritative implementation sequence is `CRM_IMPLEMENTATION_ROADMAP_v1.3.1.md` plus `Phases/PHASE_00...PHASE_14`. Do not derive phase order or completion gates from this historical section.
...
| Objective | Establish the stable SvelteKit + Cloudflare Workers with Static Assets + Supabase architecture upon which all future vertical slices will build. |
...
| Note | Keep dependencies minimal and obey `DEPENDENCY_BASELINE_v1.0.0.md`: Bun-only package management, Svelte/SvelteKit/Vite build responsibilities, exact direct pins after the Phase 1 compatibility proof, single `bun.lock`, and no autonomous dependency drift. Use strict TypeScript. The frontend must remain static-first and must never contain service-role, SendPulse, database, or webhook secrets. Use semantic design tokens rather than client-specific raw styling. Performance: static assets belong at CDN layer; business data remains cold Postgres in future phases; Redis = none; browser persistent PII cache = none; PubSub/Realtime = none for scaffold. No DB index requirements yet. Tools: filesystem read/write, shell, git; use browser only for final smoke test. STOP when clean development/build/test commands pass and architecture folders/docs exist; do not proceed into Auth or CRM resources. |
...
Cloudflare Workers            Supabase Cron
         │                            │
         └──────────────┬─────────────┘
                        ▼
                 Edge Functions
...
The technical stack is particularly attractive for small-client deployments because SvelteKit can be deployed through Cloudflare Workers with Static Assets, PostgreSQL provides durable business truth, Supabase supplies Auth/RLS/functions/scheduling/storage, Bricks gives a simple webhook boundary, and SendPulse handles transactional email.
