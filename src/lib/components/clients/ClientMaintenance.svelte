<script lang="ts">
	...
	import { clientStatusLabel } from '$lib/domain/presentation/labels';
...
<Card class="maintenance-card">
	<SectionHeader
		...
		description="Edit details or change status when needed. Ordinary viewing stays above this section."
	/>
	{#if canEditDetails}
		<details class="edit-disclosure">
			...
			<form method="POST" action="?/update" class="maintenance-form">
				...
				<div class="form-actions">
					<Button type="submit" size="sm">Save customer details</Button>
				</div>
			</form>
		</details>
	{:else}
	...
	{#if canChangeStatus}
		<details class="admin-disclosure">
			<summary>Status actions (Inactivate / Reactivate / Archive / Restore)</summary>
			<div class="status-actions">
				<p class="muted">Current status: {clientStatusLabel(client.status)}</p>
				<form method="POST" action="?/status" class="status-form">
					...
					<Input
						...
						value={value('reason')}
					/>
					<Button type="submit" size="sm" variant="secondary">Save status</Button>
				</form>
			</div>
		</details>
	{/if}
