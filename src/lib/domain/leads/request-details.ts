...
export function parseLeadRequestMessage(message: string | null | undefined): ParsedLeadRequestMessage {
...
export function shouldExpandLeadRequestDetails(input: {
  ...
  const recentDays =
    Number.isFinite(input.recentDays) && (input.recentDays ?? 0) > 0 ? input.recentDays ?? 14 : 14;
